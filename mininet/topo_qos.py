...
class QosTopo(Topo):
    def build(self, num_sims=100):
        # Switches centrais
        switch_tb = self.addSwitch('s1001')      # nome válido
        switch_middts = self.addSwitch('s1002')  # nome válido
        ...
        # Link entre ThingsBoard e MidDiTS
        self.addLink(switch_tb, switch_middts, cls=TCLink, bw=100, delay='5ms')

        for i in range(1, num_sims + 1):
            host_name = f'sim_{i:03d}'
            switch_name = f's{i}'  # <- nome canônico exigido

            # Cria switch e host
            self.addSwitch(switch_name)
            self.addHost(host_name)

            # Conecta host ao seu switch local
            self.addLink(host_name, switch_name)

            # Três links diferentes (slices) até o switch do ThingsBoard
...
if __name__ == '__main__':
    ...
    # Limpa o ambiente antes de criar
    # os.system('mn -c')
