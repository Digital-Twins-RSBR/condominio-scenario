#!/usr/bin/env python3
"""Finalized run metrics helper.

Computes weighted means, medians (excluding zero-sample sensors), P95 and
cdf<=200, and ODTE component stats. Median for T/R is computed only across
rows where the corresponding sent count is > 0 (to avoid zeros from inactive
sensors skewing the median to 0).
"""
import argparse
import csv
import glob
...
def read_per_sensor_stats(path_pattern):
    ...
    pairs: list of (mean, count) for weighted mean
    medians_with_counts: list of (median, count) aligned per-sensor only when a median exists
    """
    pairs = []
    medians_with_counts = []
            ...
            with open(path, newline='') as f:
                ...
                for row in r:
                    ...
                    if mean is not None:
                        ...
                        # try to extract median only when count>0 and median column present
                        if cval > 0:
...
def read_odte_components(odte_csv):
    """Read ODTE CSV and compute mean/median for T, R, A.

    For median_T and median_R we only consider rows where the relevant sent
    count is > 0 (sim_sent_count for s2m, middts_sent_count for m2s). This
    avoids medians being zero because many rows represent inactive sensors.
    """
        ...
        with open(odte_csv, newline='') as f:
            ...
            for row in r:
                ...
                for key in row.keys():
                    ...
                    if val in (None, ''):
                        continue
                    # T columns: require corresponding sent count > 0
                    if k.startswith('t'):
                        # s2m vs m2s
...
def main():
    ...
    # median_of_medians (optionally filtered by min-count)
    ...
    if ecdf:
        ...
        # overall median from ECDF (median over all points)
