#!/usr/bin/env python3
"""
Comparação Direta URLLC vs eMBB
Analisa as diferenças de performance entre os perfis de rede
"""
...
def analyze_test(test_dir, profile_name):
    """Análise condensada de um teste específico"""
...
def compare_profiles(urllc_data, embb_data):
    """Compara performance entre URLLC e eMBB"""
    ...
    if urllc_data['s2m_latency_avg'] < embb_data['s2m_latency_avg']:
        categories.append(("Latência S2M", "URLLC", f"{-s2m_diff_avg:.1f}ms melhor"))
    else:
        categories.append(("Latência S2M", "eMBB", f"{s2m_diff_avg:.1f}ms melhor"))
    
    # Latência M2S
    if urllc_data['m2s_latency_avg'] < embb_data['m2s_latency_avg']:
        categories.append(("Latência M2S", "URLLC", f"{-m2s_diff_avg:.1f}ms melhor"))
    else:
        categories.append(("Latência M2S", "eMBB", f"{m2s_diff_avg:.1f}ms melhor"))
    
    # ODTE
    if urllc_data['odte_general'] > embb_data['odte_general']:
        categories.append(("ODTE Geral", "URLLC", f"{-odte_diff*100:.1f}pp melhor"))
    else:
        categories.append(("ODTE Geral", "eMBB", f"{odte_diff*100:.1f}pp melhor"))
...
def compare_three_profiles(urllc_data, embb_data, best_effort_data):
    """Compara performance entre URLLC, eMBB e best_effort"""
    ...
    print(f"   ✅ Todos os perfis adequados para IoT Digital Twins")

def main():
    results_dir = Path("results")

    # Encontrar testes mais recentes (case-insensitive)
    ...
    # accept both underscores and hyphens in best_effort naming
    ...
    # Pegar os mais recentes (ordenando por nome - timestamp embutido no nome)
    ...
    # Se houver best_effort, fazer comparação tripla
